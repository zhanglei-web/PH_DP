#!/usr/bin/env python3
"""CUDA-only ordinary 100-seed evaluation for Oracle Light-FiLM checkpoints."""
...
class Predictor:
 def __init__(self,ck,dev):
  p=torch.load(ck,map_location=dev,weights_only=False);self.payload=p;self.dev=dev;self.model=OracleLightFiLMStageDiffusion(OracleLightFiLMConfig(**p['diffusion_config'])).to(dev).eval();self.model.load_state_dict(p['model']);n=p['normalization'];self.om=np.asarray(n['observation_mean'],np.float32);self.os=np.asarray(n['observation_std'],np.float32);self.am=np.asarray(n['action_mean'],np.float32);self.astd=np.asarray(n['action_std'],np.float32)
 @torch.inference_mode()
 def sample(self,state,stage,seed):
  gen=torch.Generator(device=self.dev).manual_seed(seed);physical=torch.as_tensor((state[:43]-self.om[:43])/self.os[:43],device=self.dev).unsqueeze(0);z=torch.eye(5,device=self.dev)[stage].unsqueeze(0);a=self.model.assist(physical,z,torch.zeros((1,7),device=self.dev),1.,generator=gen)[0];return (a.cpu().numpy()*self.astd+self.am).astype(np.float32)
def episode(pred,seed):
 c=CollectionConfig();env=PickPlaceEnv(render_mode=None,control_timestep=c.control_timestep_s,max_episode_steps=c.max_steps,enable_camera=False);ad=ExpertCommandAdapter(env.ik_controller,ExpertActionSpec())
 try:
  ob,_=env.reset(seed=seed,options={'randomize_arm':c.randomize_arm,'arm_joint_noise_scale':c.arm_joint_noise_scale,'randomize_object':c.randomize_object,'randomize_goal':c.randomize_goal});ad.reset(ob['ee_pose'],ob['q_obs']);s=np.r_[env.get_policy_observation(ob),np.float32(bool(ob['object_grasped']))].astype(np.float32);rew=AWACRewardV1Online(s,AWACRewardV1Config());con=0;reason='timeout';ret=0.;clip=vals=aclip=0
  for step in range(c.max_steps):
   s=np.r_[env.get_policy_observation(ob),np.float32(bool(ob['object_grasped']))].astype(np.float32);stage=int(phase_from_milestones(rew.tracker.current));raw=pred.sample(s,stage,8_000_000+seed+step);out=(raw < -1)|(raw>1);vals+=int(out.sum());clip+=int(out.any());bounded=np.clip(raw,-1,1);bounded[6]=-1. if bounded[6]<THRESH else 1.;adapt=ad.adapt(ExpertActionSpec().denormalize(bounded));aclip+=int(adapt.action_clipped);con=0 if adapt.accepted else con+1;nob,*_=env.step(adapt.joint_target);ns=np.r_[env.get_policy_observation(nob),np.float32(bool(nob['object_grasped']))].astype(np.float32);rs=rew.step(s,ns,ik_failure=con>=c.max_consecutive_ik_failures,time_limit=step+1>=c.max_steps);ret+=rs.reward;ob=nob
   if rs.task_success:reason='task_success';break
   if rs.terminated or rs.truncated:reason=rs.termination_reason;break
  m=rew.tracker.current;success=reason=='task_success';return {'environment_seed':seed,'success':success,'grasp':bool(m[0]),'lift':bool(m[1]),'transport':bool(m[2]),'place':bool(m[3]),'release':bool(m[3]),'retreat':bool(m[4]),'illegal_drop':reason=='illegal_drop','ik_failure':reason=='ik_failure_limit','timeout':reason=='timeout','termination_reason':reason,'failure_phase':None if success else ('RETREAT' if m[3] else 'APPROACH'),'episode_return':float(ret),'episode_length':step+1,'nan_count':0,'inf_count':0,'out_of_bounds_steps':clip,'out_of_bounds_values':vals,'policy_clip_steps':clip,'adapter_clip_steps':aclip}
 finally:env.close()
def main():
 p=argparse.ArgumentParser();p.add_argument('--checkpoints',nargs='+',type=Path,required=True);p.add_argument('--output',type=Path,default=ROOT/'outputs/oracle_light_film_stage_dp_v1/closed_loop');a=p.parse_args()
